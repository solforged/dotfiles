...
Item {
  ...
  function runBlank() {
    // Image readers (U.are.U 4500) stay in capture while pam_fprintd is
    // armed. libfprint's thermal model disables them after 3 minutes of
    // that, and the 250ms retry loop then keeps them latched off. Drop
    // capture when the panel goes dark; wake starts it again.
    displayBlanked = true
    fingerprintRetryTimer.stop()
    fingerprintAuthenticating = false
    if (fingerprintPam.active) fingerprintPam.abort()
  ...
  function startFingerprint() {
    if (!lockRequested || !sessionLock.secure || !fingerprintConfigured || displayBlanked) return
  ...
  PamContext {
    ...
    onError: function(error) {
      ...
      if (root.lockRequested && root.fingerprintConfigured && !root.displayBlanked) fingerprintRetryTimer.restart()
    }
  }
  ...
  Timer {
    ...
    onTriggered: {
      ...
      // Fingerprint capture is dropped in runBlank so image readers can
      // cool. Do not hold the panel up for an in-flight fingerprint PAM.
